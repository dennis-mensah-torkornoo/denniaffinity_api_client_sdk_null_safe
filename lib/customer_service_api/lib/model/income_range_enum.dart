...
class IncomeRangeEnum {
  ...
  static IncomeRangeEnum? fromJson(dynamic value) => IncomeRangeEnumTypeTransformer().decode(value);

  static List<IncomeRangeEnum>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <IncomeRangeEnum>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IncomeRangeEnum.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
...
class IncomeRangeEnumTypeTransformer {
  ...
  IncomeRangeEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
