...
class EKYCDisabilityChoiceEnum {
  ...
  static EKYCDisabilityChoiceEnum? fromJson(dynamic value) => EKYCDisabilityChoiceEnumTypeTransformer().decode(value);

  static List<EKYCDisabilityChoiceEnum>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EKYCDisabilityChoiceEnum>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EKYCDisabilityChoiceEnum.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
...
class EKYCDisabilityChoiceEnumTypeTransformer {
  ...
  EKYCDisabilityChoiceEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data != null) {
