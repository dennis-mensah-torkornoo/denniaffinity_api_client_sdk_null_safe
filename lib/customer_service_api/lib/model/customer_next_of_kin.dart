...
class CustomerNextOfKin {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CustomerNextOfKin[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CustomerNextOfKin[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CustomerNextOfKin>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CustomerNextOfKin>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CustomerNextOfKin.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CustomerNextOfKin> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = CustomerNextOfKin.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<CustomerNextOfKin>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CustomerNextOfKin>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CustomerNextOfKin.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
