...
class MobileStoreEkycKinDetailsRequest {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MobileStoreEkycKinDetailsRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MobileStoreEkycKinDetailsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MobileStoreEkycKinDetailsRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <MobileStoreEkycKinDetailsRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MobileStoreEkycKinDetailsRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MobileStoreEkycKinDetailsRequest> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = MobileStoreEkycKinDetailsRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<MobileStoreEkycKinDetailsRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MobileStoreEkycKinDetailsRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MobileStoreEkycKinDetailsRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
