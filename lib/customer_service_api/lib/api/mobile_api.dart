...
class MobileApi {
  ...
  /// * [String] id (required):
  ///
  /// * [MobileUpdateIdentificationDetailsRequest] mobileUpdateIdentificationDetailsRequest:
  Future<Response> mobileUpdateIdentificationDetailsWithHttpInfo(String id, { MobileUpdateIdentificationDetailsRequest? mobileUpdateIdentificationDetailsRequest, }) async {
    // ignore: prefer_const_declarations
    final path = r'/customers-mobile/{id}/id-update'
      .replaceAll('{id}', id);

    // ignore: prefer_final_locals
    Object? postBody = mobileUpdateIdentificationDetailsRequest;
  ...
  Future<MobileOnboardingResponse?> mobileUpdateIdentificationDetails(String id, { MobileUpdateIdentificationDetailsRequest? mobileUpdateIdentificationDetailsRequest, }) async {
    final response = await mobileUpdateIdentificationDetailsWithHttpInfo(id,  mobileUpdateIdentificationDetailsRequest: mobileUpdateIdentificationDetailsRequest, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
