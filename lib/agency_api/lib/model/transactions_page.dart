...
class TransactionsPage {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TransactionsPage[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TransactionsPage[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return TransactionsPage(
        ...
        content: TransactionDetailResponse.listFromJson(json[r'content']) ?? const [],
  ...
  static List<TransactionsPage>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TransactionsPage>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TransactionsPage.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TransactionsPage> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = TransactionsPage.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<TransactionsPage>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TransactionsPage>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TransactionsPage.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
