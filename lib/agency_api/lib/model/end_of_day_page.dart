...
class EndOfDayPage {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EndOfDayPage[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EndOfDayPage[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return EndOfDayPage(
        ...
        content: GroupedTransactionResponse.listFromJson(json[r'content']) ?? const [],
  ...
  static List<EndOfDayPage>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EndOfDayPage>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EndOfDayPage.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EndOfDayPage> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = EndOfDayPage.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<EndOfDayPage>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EndOfDayPage>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EndOfDayPage.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
