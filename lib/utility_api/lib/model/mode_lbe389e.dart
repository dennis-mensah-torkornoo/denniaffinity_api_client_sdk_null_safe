//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//


// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class MODELbe389e {
  /// Returns a new [MODELbe389e] instance.
  MODELbe389e({
    this.time,
  });

  String? time;

  @override
  bool operator ==(Object other) => identical(this, other) || other is MODELbe389e &&
     other.time == time;

  @override
  int get hashCode =>
  // ignore: unnecessary_parenthesis
    (time == null ? 0 : time.hashCode);

  @override
  String toString() => 'MODELbe389e[time=$time]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (time != null) {
      json[r'time'] = time;
    }
    return json;
  }

  /// Returns a new [MODELbe389e] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MODELbe389e? fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();
      return MODELbe389e(
        time: mapValueOfType<String>(json, r'time'),
      );
    }
    return null;
  }

  static List<MODELbe389e?>? listFromJson(dynamic json, {bool? emptyIsNull, bool? growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(MODELbe389e.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <MODELbe389e>[];

  static Map<String, MODELbe389e?> mapFromJson(dynamic json) {
    final map = <String, MODELbe389e?>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = MODELbe389e.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of MODELbe389e-objects as value to a dart map
  static Map<String, List<MODELbe389e?>?> mapListFromJson(dynamic json, {bool? emptyIsNull, bool? growable,}) {
    final Map<String, List<MODELbe389e?>?> map = <String, List<MODELbe389e>?>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = MODELbe389e.listFromJson(
            value,
            emptyIsNull: emptyIsNull,
            growable: growable,
          );
        });
    }
    return map;
  }
}

